...
var JATabs = new Class({	
	initialize: function(element, options) {
		...
		this.boxTab_H = this.el.getElement('ul.ja-tabs-title').offsetHeight;
			...
			*/
			...
			if (!this.options.fixheight) {
				if (skipAnim) {
					...
					if(!this.options.useAjax && ((this.options.position=='left') || (this.options.position=='right'))){
						...
						this.tabfx.start(Math.max(this.el.getElement('ul.ja-tabs-title').offsetHeight, this.el.getElement('div.ja-tab-panels-'+this.options.position).offsetHeight));
					}
				}
			}
		}	
	...
	update: function (text) {
		...
		this.activate (tab);		
		if((this.options.position=='left') || (this.options.position=='right')){
			this.divtitles.setStyle ('height', Math.max(this.boxTab_H, this.el.getElement('div.ja-tab-panels-'+this.options.position).offsetHeight));
		}
	}
});
